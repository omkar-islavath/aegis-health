...
const Dashboard = () => {
  ...
  const [loggedAt, setLoggedAt] = useState(getLocalISODateTime());

  const [medName, setMedName] = useState('');
  const [medDosage, setMedDosage] = useState('');
  const [medNotes, setMedNotes] = useState('');
  const [medTimeTaken, setMedTimeTaken] = useState(getLocalISODateTime());
  ...
  const handleLogSymptom = async (e) => {
    ...
    try {
      ...
      setNotes('');
      setGeneralFeeling(3);
      setLoggedAt(getLocalISODateTime());
  ...
  const handleLogMedicine = async (e) => {
    ...
    try {
      await axios.post('/api/medicines', {
        name: medName,
        dosage: medDosage,
        timeTaken: new Date(medTimeTaken).toISOString(),
        notes: medNotes
      });
      setMedName('');
      setMedDosage('');
      setMedNotes('');
      setMedTimeTaken(getLocalISODateTime());
      ...
      <div className="responsive-grid-split">
        ...
        <div className="glass-panel" style={{
          ...
          <div style={{ display: 'flex', gap: '15px' }}>
            <button onClick={() => { setLoggedAt(getLocalISODateTime()); setSymptomModal(true); }} className="btn-primary">
              <Plus size={16} />
              <span>Log Symptom</span>
            </button>
            <button onClick={() => { setMedTimeTaken(getLocalISODateTime()); setMedicineModal(true); }} className="btn-secondary">
              <Plus size={16} />
        ...
        <div className="modal-overlay" onClick={() => setSymptomModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleLogSymptom} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '12px', fontWeight: '700', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Log Date & Time</label>
                <input 
                  type="datetime-local" 
                  value={loggedAt}
      ...
      {medicineModal && (
        <div className="modal-overlay" onClick={() => setMedicineModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 style={{ fontSize: '18px', fontWeight: '800', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Pill size={18} style={{ color: 'var(--color-accent)' }} />
