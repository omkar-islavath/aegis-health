...
const initDatabase = async () => {
  ...
  if (
    ...
    console.log(
      "Cloud database environment detected. Skipping auto-creation verification.",
    );
    return;
  }
  ...
  const rootUrl =
    databaseUrl.substring(0, databaseUrl.lastIndexOf("/")) + "/postgres";
  ...
  try {
    ...
    if (res.rowCount === 0) {
      console.log(
        `Database "${dbName}" does not exist. Creating database now...`,
      );
